#coding=utf-8
# This code aim to build the network containing 9 subtasks:
# 2 main tasks, and 7  auxiliary tasks
...
with tf.Session() as sess:
    ...
    # while count<1:
        count += 1
        print("count:", count)
        for batch_id in range(0, 80):#Training Set Maximum is 124
        # for batch_id in range(0, 5):#Training Set
            ...
            sess.run(optimizer, feed_dict={x: batch_xs, y_1: batch_ys_1,
                                           y_2: batch_ys_2, y_3: batch_ys_3,
                                           y_4: batch_ys_4, y_5: batch_ys_5,
                                           y_6: batch_ys_6, y_7: batch_ys_7,
                                           y_8: batch_ys_8, y_9: batch_ys_9,
                                           keep_prob: drop_out})
            
            if step % display_step == 0:
                loss, acc1, acc2, acc3, acc4, acc5, acc6, acc7, acc8, acc9 = sess.run([cost, accuracy_1, accuracy_2, accuracy_3,
                                    accuracy_4, accuracy_5, accuracy_6, accuracy_7, accuracy_8, accuracy_9], feed_dict={x: batch_xs,
                                    y_1: batch_ys_1, y_2: batch_ys_2, y_3: batch_ys_3, y_4: batch_ys_4, y_5: batch_ys_5, y_6: batch_ys_6,
                                    y_7: batch_ys_7, y_8: batch_ys_8, y_9: batch_ys_9, keep_prob: 1.})
                print("Iter" + str(step*batch_size) + ", Minibatch Loss =" + \
                      "{:.6f}".format(loss) + ", Training Accuracy1=" + \
                      "{:.5f}".format(acc1) + ", Training Accuracy2=" + \
                      "{:.5f}".format(acc2) + ", Training Accuracy3=" + \
                      "{:.5f}".format(acc3) + ", Training Accuracy4=" + \
                      "{:.5f}".format(acc4) + ", Training Accuracy5=" + \
                      "{:.5f}".format(acc5) + ", Training Accuracy6=" + \
                      "{:.5f}".format(acc6) + ", Training Accuracy7=" + \
                      "{:.5f}".format(acc7) + ", Training Accuracy8=" + \
                      "{:.5f}".format(acc8) + ", Training Accuracy9=" + \
                      "{:.5f}".format(acc9))
            
            step += 1

    saver.save(sess, "/media/ubuntu/CZHhy/BarcodeQA/TIEcode/NewModel/ModelPretraind.ckpt")
# End of First Training Stage
...
'''
# Start of Second Training Stage
learning_rate = 0.001
batch_size = 100
display_step = 1
SIZE = 64
...
with tf.Session() as sess:
    sess.run(init)
    saver.restore(sess, "/media/ubuntu/CZHhy/BarcodeQA/TIEcode/NewModel/ModelPretraind.ckpt")
    ...
    saver.save(sess, "/media/ubuntu/CZHhy/BarcodeQA/TIEcode/NewModel/ModelPretraind2.ckpt")
# End of First Training Stage
'''
